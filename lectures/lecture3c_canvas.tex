\title{Lecture 5: Policy Optimization}
...
=&~\sum_\tau R(\tau) \nabla P^\pi(\tau) \nonumber\\
=&~\sum_\tau R(\tau) P^\pi(\tau) \nabla \log P^\pi(\tau)  \qquad\text{\bf(Homework 2's 3a)}\nonumber\\
...
\begin{align*}
    ...
    =&~\E_{s\sim d^{\pi_\theta},~ a\sim\pi_{\theta_{\text{old}}}(s)}\left[\frac{\pi_\theta(a|s)}{\pi_{\theta_{\text{old}}}(a|s)}A^{\pi_{\theta_{\text{old}}}}(s,a)\right] \qquad\text{\bf(Homework 2's 3c)} \\
