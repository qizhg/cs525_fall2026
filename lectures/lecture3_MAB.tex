...
\title{Lecture 3: Multi-Armed Bandit}
\author{Qi Zhang}
...
Designing and analyzing algorithms for full-width MDPs is non-trivial.
...
\subsection{$\epsilon_t$-greedy}
The standard $\epsilon$-greedy uses a constant $\epsilon$ which incurs linear regret.
A more general variant is to make $\epsilon$ dependent on the round number $t$, i.e.,
\begin{align*}
    &i_t \sim {\rm Uniform}([K]) \text{ w.p. $\epsilon_t$} 
    \quad \text{and} \quad 
    i_t =\textstyle\argmax_{i\in[K]}~\hat{\mu}_{i,t-1} \text{ w.p. $1-\epsilon_t$}.
\end{align*}
We call this {\em $\epsilon_t$-greedy}. 
It turns out that, with a proper choice of $\epsilon_t$ that only depends on $t$ and $K$ in a similar way to \eqref{eq:explore_then_commit-n}, we can provide a bound of $\epsilon_t$-greedy's expected regret that matches that of explore-then-commit in \eqref{eq:explore_then_commit-regret}.
In hindsight, this makes sense, because both algorithms essentially perform exploration by uniform sampling, the extent of which is controlled by $\epsilon_t$ and the exploration budget $n$ in explore-then-commit, respectively.
...
\begin{align*}
    \Pr\big(\underbrace{\abs{\hat{\mu}_{i, t-1}-\mu_i} \geq B_i(\delta_t)}_{=:E_{i,t}} \big) = \Pr\left(\textstyle\bigcup_{n=0}^{t-1}E_{i,t}^n \right)\leq 2t\delta_t
    .
\end{align*}
