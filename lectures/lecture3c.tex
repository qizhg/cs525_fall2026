...
=&~\sum_\tau R(\tau) P^\pi(\tau) \textstyle\nabla\left(\sum_{h=0}^{H-1} \log \pi(a_h | s_h)\right) \label{eq:PG_REINFORCE}\\ 
...
\paragraph{Variance reduction via a ``baseline''.}
...
\begin{align*}
    ...
    =&~\E_{s\sim d^{\pi_\theta},~ a\sim\pi_\theta(a|s)}\left[A^{\pi_{\theta_{\text{old}}}}(s,a)\right]
    ~~\text{($d^{\pi_\theta}$ is abused as the marginal state distribution)}\\
    =&~\E_{s\sim d^{\pi_\theta},~ a\sim\pi_{\theta_{\text{old}}}(a|s)}\left[\frac{\pi_\theta(a|s)}{\pi_{\theta_{\text{old}}}(a|s)}A^{\pi_{\theta_{\text{old}}}}(s,a)\right]\\
    \approx&~\E_{s\sim d^{\pi_{\theta_{\text{old}}}},~ a\sim\pi_{\theta_{\text{old}}}(a|s)}\left[\frac{\pi_\theta(a|s)}{\pi_{\theta_{\text{old}}}(a|s)}A^{\pi_{\theta_{\text{old}}}}(s,a)\right]\\
